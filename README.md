...
Thus with this dataset, our neural network model will take in the 13 inputs and output a probability as to if the patient obtained a heart failure or not. This makes our neural network a **Supervised Binary Classification Neural Network**. However, if we take a closer look at the dataset we see that some imputs range between 0 and 1, while others range between 0 and 200. Therefore if we use this dataset as it is the neural network may bias its output to the higher numerical valued inputs. Thus it is crucial that the program normalizes all data entries between the value of 0 and 1. This will eliminate this problem as all inputs will be within a shared numerical range. 
